...
class GHCommitFileIterable extends PagedIterable<GHCommit.File> {
    ...
    private static GitHubPaginatedEndpoint<GHCommitFilesPage, GHCommit.File> createEndpointIterable(GHRepository owner,
            String sha,
            GHCommit.File[] files) {
        GitHubPaginatedEndpoint<GHCommitFilesPage, GHCommit.File> iterable;
        if (files != null && files.length < GH_FILE_LIMIT_PER_COMMIT_PAGE) {
            // create a page iterator that only provides one page
            iterable = GitHubPaginatedEndpoint.fromPage(GHCommit.File.class, new GHCommitFilesPage(files));
        } else {
            GitHubRequest request = owner.root()
                    .createRequest()
                    .withUrlPath(owner.getApiTailUrl("commits/" + sha))
                    .build();
            iterable = new GitHubPaginatedEndpoint<>(owner.root()
